...
public class ZeroHood extends Command {
    ...
    /** 
     * A command to zero the hood when it is away from the hard stop.
     *
    ...
    public void initialize() {
        ...
        m_hood.runOpenLoop(-0.8);
    }

    // Called every time the scheduler runs while the command is scheduled.
    @Override
    public void execute() {
        if ((m_hood.getVelocity() < -0.3) & !m_isHoodAtVelocity) {
    ...
    public void end(boolean interrupted) {
        if (!interrupted) {
            m_hood.resetPosition(0);
        }

        m_hood.runOpenLoop(0);
